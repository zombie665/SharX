...
در استک، **Watchtower** (`sharx_watchtower`) همراه پنل اجرا می‌شود. پنل از طریق `XUI_DOCKER_UPDATER_URL` و `XUI_DOCKER_UPDATER_TOKEN` به HTTP API آن درخواست می‌دهد (پورت به بیرون published نیست) و ایمیج جدید `sharx` را با برچسب `com.centurylinklabs.watchtower.enable: "true"` می‌کشد. **دکمه به‌روزرسانی در رابط وب** معمولاً همین مسیر است. در production مقدار `WATCHTOWER_HTTP_API_TOKEN` را در `.env` تنظیم کنید (مثلاً `openssl rand -hex 24`).
...
docker compose up -d
```
* **با اسکریپت** — `pull` و ساختن دوباره `sharx` و `watchtower`:
```bash
sudo bash install.sh
# 2) Update Panel
```
...
4. اتصال به inbound و مدیریت روزمره در **پنل وب** (**نودها**، **جغرافیا**). `install.sh` فقط پنل و دیتابیس را می‌آورد.

جزئیات: [sharx-code/node/README.md](../sharx-code/node/README.md).
### پیکربندی پیشرفته
...
**سرویس worker (نه پنل):** مسیر متداول **pairing** با **`SECRET_KEY`** + **`PANEL_URL`** از مودال پنل است؛ [sharx-code/node/README.md](../sharx-code/node/README.md).
