...
- **Импорт из SQLite**: встроенный инструмент **SQLite → PostgreSQL** для старых дампов 3XUI (`web/service/migration.go`).
...
В стеке рядом с панелью поднимается **Watchtower** (`sharx_watchtower`): панель по переменным `XUI_DOCKER_UPDATER_URL` / `XUI_DOCKER_UPDATER_TOKEN` обращается к его HTTP API (порт **не** пробрасывается наружу) и инициирует подтягивание нового образа `sharx` с меткой `com.centurylinklabs.watchtower.enable: "true"`. Именно этот путь обычно использует **кнопка обновления в веб-интерфейсе**. Для production задайте один и тот же секрет в `.env` для `WATCHTOWER_HTTP_API_TOKEN` (например: `openssl rand -hex 24`).

Альтернативы без UI:

- **Через Docker Compose** в каталоге, где лежит `docker-compose.yml`:
  ```bash
  docker compose pull
  docker compose up -d
  ```
- **Через скрипт** — `pull` и пересоздание контейнеров `sharx` и `watchtower`:
  ```bash
  sudo ./install_ru.sh
  # 2) Обновить панель
  ```

Контейнер панели поднимается с новым образом; данные **PostgreSQL** на примонтированном томе обычно **сохраняются**.
...
4. Привязка узлов к inbound, география и обслуживание — **в веб-панели** (**Ноды**, **География**). Скрипт `install_ru.sh` поднимает только панель и БД, не worker.

Подробно про переменные, сеть и типовой compose: **[sharx-code/node/README.md](../sharx-code/node/README.md)**.
...
**Worker-узел (не панель):** типовая выкладка — **pairing** через **`SECRET_KEY`** + **`PANEL_URL`** из модалки панели; см. [sharx-code/node/README.md](../sharx-code/node/README.md).
